...
namespace WhatsAppBulkSender
{
    internal sealed class WhatsAppSender : IDisposable
    {
        ...
        public WhatsAppSender(bool headless)
        {
            ...
            var userDir = Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData) + @"\WhatsAppBulkSender\ChromeProfile";
...

        {
...

            {
...

                {
...

                    {
                        ...
                        // if clicking failed we continue with the checks below
...

            {
                ...
                var composerCandidates = _driver.FindElements(By.CssSelector("footer div[contenteditable='true'][data-lexical-editor='true']"));

                IWebElement? composer = composerCandidates.FirstOrDefault();

                if (composer == null)

                    composer = _driver.FindElements(By.CssSelector("footer [data-testid='conversation-compose-box-input']")).FirstOrDefault();

                if (composer == null)

                    composer = _driver.FindElement(By.CssSelector("footer div[contenteditable='true']"));
                ...
                bool sent = await WaitUntilAsync(() => CountOutgoingMessages() > outgoingBefore, TimeSpan.FromSeconds(10), ct);
        ...
        // Synchronous small helper used for quick verification
